...
> **Revision note (2026-09-17, Gate 1 revision cycle):** Since this brief was first produced, the user removed the marketplace-era planning artifacts this brief's assumptions log had referenced as still-standing facts — including `stack/stack-proposal.md` (the document that had recorded an approved, founder-locked Node.js/Express stack decision), `stack/rules/base-rules.md`, `stack/rules/client-rules.md`, `docs/adr/project-brief.md`, and two stale `.orchestration/runs/` directories from the old PRD/UX phases. `stack/` now contains only a generic, mechanism-only `README.md`; `docs/adr/` now contains only the orchestration ADR. This revision re-frames the stack-pivot open item (see Key Open Items below and `addendum.md` §2.4 item 1 / §4 item 4) accordingly: there is no longer a competing approved stack decision on record for the Flutter/FastAPI/Postgres direction to conflict with. This does not resolve the stack question — it remains open for the Architect phase and Gate 3 — it only removes a stale factual claim that a locked, conflicting decision still existed. No other content changed except where noted in `addendum.md`.
...
- **[ASSUMPTION]** A stack direction toward Flutter + FastAPI (Python) + Postgres is under discussion (from a prior-session architecture conversation). As of this revision, this is the only stack signal on record for this project — the repository no longer contains a competing, approved stack decision to reconcile against (the marketplace-era `stack/stack-proposal.md` that once recorded a founder-locked Node.js/Express decision has been removed from the repository by the user). This direction is still unconfirmed, however — a stated leaning, not something the user has formally approved as this project's locked stack — and remains a normal open item for the Architect phase (Phase 3) to formally evaluate and for the user to approve at Gate 3, the same as any stack proposal would require.
