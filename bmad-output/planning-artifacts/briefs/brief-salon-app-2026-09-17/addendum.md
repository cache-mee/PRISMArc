...
> **Revision note (2026-09-17, Gate 1 revision cycle):** Since this addendum was first produced, the user removed the marketplace-era planning artifacts referenced in §2.4 item 1 and §4 item 4 below as still-standing facts: `stack/stack-proposal.md` (which had recorded an approved, founder-locked Node.js/Express stack decision), `stack/rules/base-rules.md`, `stack/rules/client-rules.md`, `docs/adr/project-brief.md`, and two stale `.orchestration/runs/` directories from the old PRD/UX phases. `stack/` now contains only a generic, mechanism-only `README.md` (unchanged, not product-specific); `docs/adr/` now contains only the orchestration ADR (unchanged, not product-specific). This revision re-frames §2.4 item 1 and §4 item 4 from "direct conflict with a founder-locked decision" to "normal open item, no competing approved decision on record" — the stack choice itself remains open for the Architect (Phase 3) and the user (Gate 3), unchanged in substance; only the stale factual claim that a locked, conflicting decision still existed has been corrected. §2.4 item 5 and §4 item 9 (Postgres) have had a dangling reference to the deleted document removed. All other rows and sections were checked against the current repository state and left unchanged where still accurate.
...
1. **Stack direction (Flutter + FastAPI + Postgres) — a normal open item, not a conflict.** The user is currently leaning toward Flutter for the chat frontend, FastAPI (Python) for the backend API layer, and Postgres for the database, per the prior-session architecture discussion. As of this revision, this repository no longer contains any competing, approved stack decision: the marketplace-era `stack/stack-proposal.md` (which had specified Node.js + Express (TypeScript) as founder-locked, "not open for re-evaluation at MVP") has been deleted from the repository by the user — removed, not quietly superseded — and `stack/` now holds only a generic, mechanism-only `README.md` with no product-specific stack content. There is therefore no standing, locked decision for the Flutter/FastAPI/Postgres direction to conflict with. This does not make the direction confirmed, however: it remains a stated leaning from a prior-session discussion, not something the user has formally approved as *this* project's locked stack. It is a normal open item for the Architect phase (Phase 3) to formally evaluate and for the user to approve at Gate 3 — the same bar any stack proposal would need to clear — with no founder-override urgency attached, since that urgency was specifically about overriding an existing approved decision, which no longer applies.
...
| 4 | Stack direction (Flutter + FastAPI + Postgres) — stated leaning, not yet formally approved as this project's locked stack | Open — normal item; no competing approved decision remains on record (the marketplace-era `stack/stack-proposal.md` was deleted by the user, not superseded) | Prior-session architecture discussion | Architect (Phase 3 evaluation), then user approval at Gate 3 |
...
| 9 | Postgres as DB starting point | Resolved as a starting point (not re-opened as "any possible DB"); formal confirmation still owed | Prior-session architecture discussion — the user's own stated starting point, standing alone (no other repository document records a competing or corroborating DB decision) | Architect (to formally confirm) |
...
No item above has been resolved unilaterally by this brief where it constitutes an architecture, technical, or UX decision — those remain open for the Architect or UX Designer, or for the human where a business decision is required (items 1, 2, 11). Item 4 (stack direction) is a normal Architect-phase technical item — evaluated by the Architect at Phase 3 and approved by the user at Gate 3 like any other stack proposal — not a special human-sign-off item; the founder-override urgency it previously carried applied only to overriding an existing approved decision, and no such decision remains on record in this repository.
